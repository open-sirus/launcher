...
export const StatusBarText = () => ({
  ...
  props: {
    sirus: {
      default: object<IRealm>(
        'sirus',
        { id: 5, name: 'Sirus x10 - 3.3.5а+ ', isOnline: true, online: 48 },
        'realms'
      ),
    },
    scourge: {
      default: object<IRealm>(
        'scourge',
        { id: 9, name: 'Scourge x2 - 3.3.5a+', isOnline: true, online: 2050 },
        'realms'
      ),
    },
    frostmourne: {
  ...
  template:
    '<status-bar :realms="[sirus, scourge, frostmourne, neltharion]" :online="online" />',
})
